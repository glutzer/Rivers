...
public class MuddyGravelBlock : Block
{
    ...
    public override void OnLoaded(ICoreAPI api)
    {
        ...
        ids.Add(api.World.GetBlock(new AssetLocation("waterwheels:alluvialblock-blueclay")).Id);
        ids.Add(api.World.GetBlock(new AssetLocation("waterwheels:alluvialblock-fireclay")).Id);
    ...
    public override void OnServerGameTick(IWorldAccessor world, BlockPos pos, object extra = null)
    {
        ...
        if (deposit > threshold)
        {
            ...
            world.BlockAccessor.ExchangeBlock(types[(int)(type * types.Length)], pos);
        }
    }
    ...
    public void ChunkColumnGeneration(IChunkColumnGenerateRequest request)
    {
        ...
        for (int x = 0; x < 32; x++)
        {
            for (int z = 0; z < 32; z++)
            {
                ...
                TickBlock(new BlockPos(32 * request.ChunkX + x, height, 32 * request.ChunkZ + z), chunks[0].MapChunk);
            }
        }
    }

    public void TickBlock(BlockPos pos, IMapChunk chunk)
    {
        ushort[] riverDistance = chunk.GetModdata<ushort[]>("riverDistance");
        bool valid = riverDistance[(pos.Z % 32 * 32) + (pos.X % 32)] < 5;
